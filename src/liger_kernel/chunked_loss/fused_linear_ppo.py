    ...
    def forward(
        ...
        def fused_fwd_bwd(
            ...
            with torch.enable_grad():
                ...
                chunk_loss, chunk_metrics = compute_loss(
                    input_chunk,  # arg 0
                    weight_local,  # arg 1
        ...
        if compiled:
            # The GRPO selective path uses a custom autograd Function, which integrates cleanly with
            # torch.autograd.grad here but not with torch.func.grad_and_value.
            compiled = False
